package com.vivek.wo.common.view.wheelpicker;
...
public class WheelPicker extends View implements IDebug, IWheelPicker, Runnable {
    ...
    public WheelPicker(Context context, AttributeSet attrs) {
        ...
        mItemTextSize = a.getDimensionPixelSize(R.styleable.WheelPicker_wheel_item_text_size,
                24);
        mSelectedItemSize = a.getDimensionPixelSize(R.styleable.WheelPicker_wheel_selected_item_text_size, -1);
        ...
        mIndicatorSize = a.getDimensionPixelSize(R.styleable.WheelPicker_wheel_indicator_size, 2);
...
//        mDrawnCenterY = (int) (mWheelCenterY - ((mPaint.ascent() + mPaint.descent()) / 2));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int drawnDataPos = drawnDataStartPos + mSelectedItemPosition,
            ...
            if (mSelectedItemSize != -1) {
                if (mDrawnItemCenterY <= mWheelCenterY + mItemHeight && mDrawnItemCenterY >= mWheelCenterY - mItemHeight) {
                    int scaleTextSize = (int) (mSelectedItemSize + Math.abs((mDrawnItemCenterY - mDrawnCenterY) * 1f / mItemHeight) * (mItemTextSize - mSelectedItemSize));
                    scaleTextSize = Math.max(scaleTextSize, mItemTextSize);
                    mPaint.setTextSize(scaleTextSize);
                } else {
                    mPaint.setTextSize(mItemTextSize);
                }
            }
            float baseLine = mDrawnItemCenterY - ((mPaint.ascent() + mPaint.descent()) / 2);
...
//            int drawnCenterY = isCurved ? mDrawnCenterY - distanceToCenter : mDrawnItemCenterY;
            int drawnCenterY = isCurved ? mDrawnCenterY - distanceToCenter : (int) baseLine;
    ...
    public void setSelectedItemPosition(int position, final boolean animated) {
        ...
        if (animated && mScroller.isFinished() && mItemHeight != 0) { // We go non-animated regardless of "animated" parameter if scroller is in motion
            ...
            mScroller.startScroll(0, mScrollOffsetY, 0, (-itemDifference) * mItemHeight);
            mHandler.post(this);
        } else {
            if (!mScroller.isFinished())
                mScroller.abortAnimation();
